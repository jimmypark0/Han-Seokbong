...
---------------------------------------------------------------------------*/
extern PREFIX int get_stems_TS_new(		// 어절 or 문장 단위 인터페이스
	char *sent,	// KSC5601 input sentence
	char *termlist,	// list of keywords -- 탭 문자로 구분
	char *filename_KLT2000_ini);	// option file name
...
//	while (fscanf(fpin, "%s", sent)) {	if (feof(fpin)) break;
	while (fgets(sent, SENTSIZE-1, fpin)) {	if (feof(fpin)) break;
		...
		n = get_stems_TS_new(sent, termlist, "./hdic/KLT2000.ini");
		printf("[[%s]]\n", sent);	// 입력 문장 or 어절 1개
	...
	GOODBYE(2013);	/* good-bye message: 'header/ham-ndx.h' */
	return 0;
}
/*------------------------------ end of index.c ----------------------------*/
